...
# 0. 配置与全局常量
# ==========================================
# ⚠️ 请确保此路径正确，指向你的全量码表文件
ALL_PATH_FILE = "../node_index/merge_path.jsonl" 
    ...
    with open(jsonl_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            try:
                ...
                # L1
                l1_raw = entry["first_chapter"] # Strict access
...
def calculate_chapter_accuracy(data_file_path):
    ...
    print(f"🎯 模式: 全量数据统计 (final_node) + 多层级(L1-L4)验证")
    print(f"{'='*90}")
    ...
    with open(data_file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # 提取预测代码 (从 final_node 获取)
            final_node = data['final_node']
            predict_code = ""
            if isinstance(final_node, dict):
                predict_code = str(final_node['node_id']).strip()
            ...
            # === 3. 获取预测路径 ===
            # 若 predict_code 为空或不在码表中，predict_path 为 None
            ...
            if predict_path:
                ...
                # --- Level 4 Check (Code) ---
                if predict_code == answer_code and answer_code != "":
                    is_l4_ok = True
    ...
    if total_all > 0:
        ...
        print("-" * 75)
        print(f"{'总计 (Level 4)':<25} {total_all:<10} {global_stats['l4_correct']:<10} {overall_acc:.2f}%")
...
if __name__ == "__main__":
    ...
    data_files = [
        "./paper_test/deepseek/deepseek_paper.jsonl",
    ]
